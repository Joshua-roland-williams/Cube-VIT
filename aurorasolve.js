...
const animationEngine = ( () => {
  ...
  class Controls {
    ...
    applyMove() {
        let moveSequence = document.getElementById("moveInput").value.trim();
        if (!moveSequence) return;
    
        let moves = moveSequence.split(" "); // Split moves by space (e.g., "U R U'")
        
        moves.forEach(move => {
            game.controls.keyboardMove('LAYER', { axis: move[0].toLowerCase(), angle: getMoveAngle(move) }, () => {
                game.storage.saveGame(); // Save state
            });
        });
    ...
    getMoveAngle(move) {
        if (move.includes("'")) return -Math.PI / 2;
        if (move.includes("2")) return Math.PI;
        return Math.PI / 2;
    }
